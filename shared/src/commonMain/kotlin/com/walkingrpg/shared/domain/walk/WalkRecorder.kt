...
interface WalkRecorder {
     ...
     *
     * **畳まれたセッションはすべてここに来る**：手動停止・自宅到着による自動終了・
     * 測位ストリームの終了（エラー）に加えて、[start] の頭で畳まれる
     * 開きっぱなしのセッション（クラッシュ後の `ABANDONED`）も含む。
     * 畳み方ごとに導出（map matching → 成長）の呼び出しを書き分けると、
     * 経路が増えるたびに書き漏らす。[state] の `isRecording` の立ち下がりでも
     * 代用できそうに見えるが、それだと「どのセッションが終わったか」が取れないし
     * （すでに `sessionId` は `null` に落ちている）、`ABANDONED` は
     * そもそも `isRecording` を経由しない。
     *
